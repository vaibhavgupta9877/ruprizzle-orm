...
The workspace version is `1.0.0-rc.1`; the tag should be moved to current HEAD and crates.io publish is pending the W6-04 release-candidate cut and W6-05 re-scoring. The W4-02 48-hour soak is waived. The pre-RC production-readiness score was re-scored to 92 / 100 in `ProjectPlan/ProductionReadiness.md` §15, §16 (independent re-run, 2026-08-21) superseded that with 87 / 100 after finding that `cargo test --workspace` failed reproducibly with Postgres attached, and **§17 re-scores to 89 / 100** now that the defect is fixed: the suite runs each Postgres test in a private schema, is green end to end (476 passed / 0 failed), leaves no tables in `public`, and leaks no schemas. The RC feedback window is still pending.
...
| Production-readiness score (last self-assessment) | **89 / 100** (`ProjectPlan/ProductionReadiness.md` §17, after closing the §16 test-isolation and `metrics`-coverage gaps); the live RC rescoring (≥ 92) must still be rerun once `1.0.0-rc.1` is on crates.io and the feedback window has run |
...
- Resumable `rusqlite` soak evidence accepted (15.56 h / 1.46 B ops / 0 errors); the full 48-hour target is waived.
...
- **Soak**: a resumable 15.56 h / 1.46 B ops / 0-errors `rusqlite` run is recorded and the 48-hour W4-02 target is waived. (`docs/SoakReport.md`)
...
5. ✅ **W4-02 soak gate waived** — the resumable `rusqlite` soak has accumulated 15.56 h (56,028.6 s), 1,464,277,925 operations, and 0 errors; the remaining 48-hour target is accepted as sufficient. `docs/SoakReport.md` records the decision.
...
7. ✅ **Bump the workspace version to `1.0.0-rc.1`** — completed; tag and crates.io publish are pending W6-04/W6-05. The W4-02 soak gate is waived.
