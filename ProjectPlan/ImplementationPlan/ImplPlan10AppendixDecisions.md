...
Architecture decision records have been moved to [docs/adr/index.md](../../docs/adr/index.md).
...
| R2 | IR churn after P0 forces rework across crates | 40% | 🔴 high | Vaibhav Gupta owns the IR; changes require an ADR entry; all crates consume IR through accessors, never field access patterns spread widely | any IR change after G3 |
...
## Testing exception — DB-backed diff round-trip proptest

DB-backed diff round-trip proptest is verified on Postgres only in PR-13; SQLite
coverage is deferred because the property schema is pinned to the Postgres dialect
by the brief.
