...
Add a `## Observability` section to `docs/QueryGuide.md` showing a `tracing-subscriber`
...
list in `docs/KnownLimitations.md`.
...
Add a `## Connection pooling` section to `docs/QueryGuide.md` with a worked example
sizing `max_connections` against Postgres's own limit.

```bash
git add crates/runtime/src/pool.rs crates/runtime/src/lib.rs \
        crates/runtime/tests/pool_config.rs docs/QueryGuide.md
...
Add a note to `docs/QueryGuide.md` under error handling explaining the policy.

```bash
git add crates/runtime/src/error.rs crates/runtime/tests/error_redaction.rs docs/QueryGuide.md
...
**Status: COMPLETE.** Verified 2026-08-11: PR-08 wires the real generated-code gate and
`cargo xtask harden` (including panic budgets and the deny check), PR-09 adds a Windows/
macOS/Ubuntu test matrix and runs tests at MSRV, and PR-10 adds a standalone `cargo-deny`
job and Dependabot. `cargo deny check advisories licenses bans sources` exits clean.
...
```yaml
  generated-code:
    ...
    steps:
      ...
      #
      # The two tests share `target/generated-check`; running them together
      # races. xtask runs them sequentially.
...
Expected: both tests pass, roughly 22 seconds.

- [x] **Step 3: Add the harden job**
...
- [x] **Step 4: Make the panic audit an actual gate**

`panic_audit` in `xtask/src/main.rs:166` prints every `unwrap()`, `expect()`,
`panic!`, `todo!`, and `unimplemented!` it finds and then always returns `Ok(())`. It is
presented as a hardening step but can never fail the build, so wiring `xtask harden`
into CI (Step 3) would add a job that reports problems nobody is forced to read.
...
> **Implementation notes (as executed):**
>
> - The `generated-code` job uses `cargo xtask examples` because the two ignored
>   tests cannot run in the same `cargo test` invocation: they both write
>   `target/generated-check` and the pedantic test depends on the materialisation
>   test.
> - `xtask harden` now runs `cargo publish --dry-run --no-verify`; without
>   `--no-verify` `cargo publish` resolves workspace path dependencies against the
>   version already on crates.io, which is stale until the actual release.
...
- [x] **Step 1: Make the test job a matrix**
...
- [x] **Step 2: Make the MSRV job run tests**
...
- [x] **Step 3: Verify locally on this machine**
...
- [x] **Step 1: Add a standalone deny job**
...
- [x] **Step 3: Verify the deny configuration passes today**

Run: `cargo deny check advisories licenses bans sources`

If `cargo-deny` is not installed: `cargo install cargo-deny --locked`

Expected: passes. The configuration needed updates for cargo-deny 0.20:
`unsound` no longer accepts `warn`, so it is set to `none` for the alpha and
`unmaintained` is scoped to workspace direct dependencies. `Unicode-3.0` and `Zlib`
were added to the licence allowlist. The internal `ruprizzle-testkit` workspace
dependency is a `publish = false` path/dev-dependency, so `allow-wildcard-paths = true`
was added to `[bans]` to keep `wildcards = "deny"` from flagging it. The check emits
warnings for duplicate crate versions but exits clean.
...
- Modify: `docs/KnownLimitations.md`
...
Add to `docs/KnownLimitations.md` under *Current alpha*:
...
```bash
git add ProjectPlan/ImplementationPlan/ImplPlan10AppendixDecisions.md docs/KnownLimitations.md
