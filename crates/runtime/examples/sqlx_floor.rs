    ...
    Fut: std::future::Future<Output = usize>,
{
    ...
    if rows > 0.0 {
        println!(
            "{label:<44} {us:>9.1} us/op  {:>7.3} us/row  ({checksum})",
            us / rows
        );
    } else {
...
async fn main() -> Result<(), Box<dyn std::error::Error>> {
    ...
    bench(
        "execute      SELECT * (rows discarded)",
        300,
        0.0,
        || async {
            pool.execute("SELECT id, email, age FROM users")
                .await
                .unwrap()
    ...
    bench("fetch_all    SELECT * (rows kept)", 300, 1000.0, || async {
        sqlx::query("SELECT id, email, age FROM users")
            .fetch_all(&pool)
            .await
            .unwrap()
            .len()
    ...
    .await;
    bench(
        "fetch_all    SELECT id only (1 col)",
        300,
        1000.0,
        || async {
    ...
    .await;
    bench(
        "fetch_all    SELECT email only (1 text col)",
        300,
        1000.0,
        || async {
    ...
    bench("fetch_all    LIMIT 1", 300, 0.0, || async {
        sqlx::query("SELECT id, email, age FROM users LIMIT 1")
            .fetch_all(&pool)
            .await
            .unwrap()
            .len()
    ...
    bench("execute      posts (rows discarded)", 60, 0.0, || async {
        pool.execute("SELECT id, author_id, title FROM posts")
            .await
            .unwrap()
            .rows_affected() as usize
    })
    ...
    bench(
        "stream+fold  posts (decode, drop each row)",
        60,
        10000.0,
        || async {
