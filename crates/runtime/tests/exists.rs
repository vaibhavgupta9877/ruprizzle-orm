//! Correlated `EXISTS` / `NOT EXISTS` round-trips over the dual-database `both_dbs!` harness.

use ruprizzle::sqlx::{self, ColumnIndex, Decode, Type};
use ruprizzle::{Column, Filter, Model, SelectQuery};
...
fn expected_exists_sql(backend: &str) -> &'static str {
    match backend {
        "postgres" => {
...
fn expected_not_exists_sql(backend: &str) -> &'static str {
    match backend {
        "postgres" => {
...
both_dbs! {
    setup = SETUP_SQL;
    async fn exists_to_sql_and_fetch(db: TestDb) {
        let sub = SelectQuery::<Post>::new(db.pool())
            .columns(POST_ID)
            .filter(POST_AUTHOR_ID.correlated_to(USER_ID));
...
both_dbs! {
    setup = SETUP_SQL;
    async fn not_exists_fetch(db: TestDb) {
        let sub = SelectQuery::<Post>::new(db.pool())
            .columns(POST_ID)
            .filter(POST_AUTHOR_ID.correlated_to(USER_ID));
        let q = SelectQuery::<User>::new(db.pool())
