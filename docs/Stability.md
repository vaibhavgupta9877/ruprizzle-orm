...
`1.5.1` is the first stable release after `1.0.0-rc.1`. Its public API differs from the RC
in ways `cargo-semver-checks` classifies as major: new variants on exhaustive enums, new
fields on constructible structs, three diagnostic enums made `#[non_exhaustive]`, and four
internal modules made `#[doc(hidden)]`. As the release-candidates section above allows,
these are accepted as RC → stable changes rather than forcing `2.0.0`. Two conditions
apply:
