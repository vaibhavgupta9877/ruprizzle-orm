...
The product promises one identical Rust API across Postgres and SQLite, with the
backend chosen by URL scheme at runtime. `sqlx::Any` is the only sqlx facility
that provides this without generating a separate client per dialect.

## Decision

Route all runtime queries through `sqlx::Any`.
...
Any of: a third dialect is added; benchmarks (PR-12) show the text round-trip
exceeding the P8-02 thresholds; or users need Postgres arrays. The exit is
generating dialect-specific native code paths behind a feature flag, which is
additive but costs the runtime-selection property.
